...
@Optimize
@SecondOrderOptimize
fun target(a: DScalar, L: Int, nonActive: FloatScalar): DScalar {
    var i = 0
    var s = a
    while (i < L) {
        val k = nonActive + 2f
...
fun nonOptimal_target(a: DScalar, L: Int, nonActive: FloatScalar): DScalar {
    var i = 0
    var s = a
    while (i < L) {
        val k = nonActive + 2f
...
fun box(): String {
    ...
    for (element in listOf(constant1, constant2).withIndex()) {
        ...
        val primal_derivative = primalAndForwardDerivative(
            x = x,
            f = { z: DScalar -> primalAndReverseDerivative(z, { xx: DScalar -> target(xx, c, x) }).second }
        )
        ...
        val primal_derivative_expectation = primalAndForwardDerivative(
            x = x,
            f = { z: DScalar -> primalAndReverseDerivative(z, { xx: DScalar -> nonOptimal_target(xx, c, x) }).second }
        )
