...
fun box(): String {
    ...
    val (primal, derivative) = primalAndForwardDerivative(
        x = value,
        f = { z: DScalar -> primalAndReverseDerivative(z, { xx: DScalar -> logProbOf(xx, loc, scale) }).second }
    )
    val (expected_primal, expected_derivative) = primalAndForwardDerivative(
        x = value,
        f = { z: DScalar -> primalAndReverseDerivative(z, { xx: DScalar -> vanillaLogProbOf(xx, loc, scale) }).second }
    )
    val tol = 0.000001f
